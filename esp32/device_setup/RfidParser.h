...
class RfidParser {
public:
  static String parseRfidFrame(const uint8_t* buf, int len) {
    if (len < 6) { 
    ...
    if (type == RFID_TYPE_RESPONSE && cmd == RFID_CMD_GET_INFO) {
      ...
      uint16_t power = (buf[5] << 8) | buf[6];
      String res = String("Power: ") + power + " (" + (power/100) + " dBm)";
      Serial.println("[rfid] " + res);
      return res;
    }
      ...
      // Approx Freq Calc
      float freqCN = 920.125 + (chIndex * 0.25);
      float freqUS = 902.75 + (chIndex * 0.5);
      float freqEU = 865.1 + (chIndex * 0.2);
      
      String res = "Channel Index: " + String(chIndex) + " (CN~" + String(freqCN, 3) + "MHz, US~" + String(freqUS, 3) + "MHz, EU~" + String(freqEU, 3) + "MHz)";
      Serial.println("[rfid] " + res);
      return res;
    }
    ...
    else if (type == RFID_TYPE_RESPONSE && cmd == RFID_CMD_GET_DEMOD_PARAM) {
      // Get Demodulator Params Response
      // PL=4. Data=Mixer(1) IF(1) Thrd(2)
      if (pl != 4) return "Error: GetDemod PL!=4";
      uint8_t mixer = buf[5];
      uint8_t ifAmp = buf[6];
      uint16_t thrd = (buf[7] << 8) | buf[8];
      String res = "DemodParam: Mixer=" + String(mixer, HEX) + " IF=" + String(ifAmp, HEX) + " Thrd=" + String(thrd, HEX);
      Serial.println("[rfid] " + res);
      return res;
    }
    else if (type == RFID_TYPE_RESPONSE && cmd == RFID_CMD_SET_DEMOD_PARAM) {
      // Set Demodulator Params Response
      // PL=1. Parameter=00 (Success)
      if (pl != 1) return "Error: SetDemod PL!=1";
      uint8_t status = buf[5];
      String res = String("SetDemodParam: ") + (status == 0x00 ? "Success" : "Fail");
      Serial.println("[rfid] " + res);
      return res;
    }
